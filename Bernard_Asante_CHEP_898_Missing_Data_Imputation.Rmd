...
```{r}
data <- data %>%
          select(where(~sum(is.na(.x))/length(.x) < 0.17)) 
...
# Selecting 11 columns randomly from the dataset due to high computation power required for the entire dataset
health_data <- data %>% 
  
  select(ID,HS_GEN_HEALTH,NUT_VEG_QTY, NUT_FRUITS_QTY,PA_TOTAL_SHORT,DIS_ASTHMA_EVER,DIS_DIAB_EVER)

write.csv(health_data, "health_data.csv")
```
...
```
This is to display only the missing values. 
...
```
This suggest that the data is not missing completely at random. This is because, the null hypothesis states that the data is missing completely at random, pvalue of 0 means that the data is not missing completely at random.
...
print(missing_Pa_total)

glm(is.na(PA_TOTAL_SHORT) ~ HS_GEN_HEALTH + NUT_FRUITS_QTY + DIS_ASTHMA_EVER, 
    data = health_data, 
    family = binomial) %>% 
  summary()

```
It can be observed that all the variables has a pvalue < 0.002 indicating they are significant predictors of the missingness in the pa_total_short variable. Again this is an assumption from the model and can not be proven with high certainty
...
I can be observed that the median for non imputed pa short column  is 1782 and increased to 2379 after mean imputation. 
...
data_mice <- mice(health_data, m = 5, maxit = 5)
...
We can see the median of Physical activity total is around 1700 minutes.
